 ...
 * re-render. Heuristic-only when no ANTHROPIC_API_KEY is set (a suspiciously
 * small screenshot usually means a blank page), upgraded to an actual vision
 * critique from Claude when a key is available. The loop is intentionally
 * bounded to one auto-fix attempt (see orchestrator.ts's VISUAL_REVIEW step)
 * -- this is a safety net for the AI's own output, not an open-ended retry
 * that could loop forever against a page that's fine but merely unusual.
 */
